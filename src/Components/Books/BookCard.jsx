...
const BookCardList = ({ books, deleteBook, handleEditBook, handleDetailModal,handleFavourite }) => {
  return (
    <>
      <div className="flex flex-row flex-wrap justify-center mt-2 mx-5 ">
        {books.map((book, index) => (
          <div key={index} className="flex gap-2 m-2 w-96  autoFlex shadow-md border p-3 rounded-md">
            <img
            ...
            <div className="flex flex-col justify-between w-full">
              ...
              {book.isFavourite ? <FaHeart className="text-[#FFAE00] text-xl"/> : <FaRegHeart className="text-[#FFAE00] text-xl" /> }
              </div>
                <MdDeleteForever
                  onClick={() => {
                    deleteBook(book.id);
